...
**Phase 3 — Risk management (Notebooks 10-11).** Take the same PCA model and use it the way Litterman & Scheinkman originally proposed — to **hedge** factor exposures rather than to predict them. Compute DV01 and PC factor exposures for a $100M long-30Y portfolio, solve a linear system for the hedge notionals, then backtest four variants (unhedged / PC1 only / PC1+PC2 / PC1+PC2+PC3) to quantify variance reduction. The result: **96.03% reduction in daily P&L variance** with a 3-instrument hedge — exactly the explained-variance budget the PCA started with. Same factors, opposite outcome from Phase 2.
...
* **Phase 3(Notebooks 10-11)**: 同じ PCA を **リスク管理ツールとして** 使う。仮想ポートフォリオの PC factor exposures を計算 → 連立方程式でヘッジ比率を解く → 4 variant のバックテストで **96.03% の分散削減** を確認(残差 3.97% は PCA の未説明分散 3.8% とほぼ完全に一致)。Phase 2 と同じ PCA、逆方向の結果。

`src/yield_curve_pca/` に installable package、52 件の pytest、CI/CD まで揃えた reproducible な構成。
...
>
> 最も重要な発見として、**同じ PCA を予測ではなくリスク管理に使うと結果が劇的に変わる**。$100M 30Y ロングのポートフォリオに対して、PC1+PC2+PC3 を 3 つの bond ({3M, 5Y, 20Y}) でヘッジすると、日次 P&L 分散を **96.03% 削減**(残り 3.97% は PCA の未説明分散 3.8% と数学的にほぼ一致)。**「PCA は予測機ではなくリスク分解器として最強」**という、Litterman & Scheinkman の元論文タイトル(*Common Factors Affecting Bond Returns* — *Predictions* ではない)が示唆する非対称性を、自分の手で定量実証した。
>
